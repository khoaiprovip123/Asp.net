...
namespace DienThoaiDiDong
{
    public class XCart
    {
        public List<CartItem> AddCart(CartItem cartitem, int productid)
        {
            ...
            {
                listcart = (List<CartItem>)System.Web.HttpContext.Current.Session["MyCart"];// ép kiểu
                                                                                            //Kiểm tra prodctid co trong danh sách hay k
                if (listcart.Where(m => m.ProductId == productid).Count() != 0)
                {
                    ...
                    System.Web.HttpContext.Current.Session["MyCart"] = listcart;//đã có
                }
                else
                {
                    listcart.Add(cartitem);
                    System.Web.HttpContext.Current.Session["MyCart"] = listcart;  //ckua có
                }
